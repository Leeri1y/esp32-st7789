...
const uint16_t GRID_COLOR = TFT_DARKGREEN;
const uint16_t BG_COLOR = TFT_BLACK;
const uint16_t WAVE_COLOR = TFT_GREEN;
const uint16_t DIGITAL_COLOR = TFT_YELLOW;
const uint16_t TEXT_COLOR = TFT_GREEN;
const uint16_t PAUSE_COLOR = TFT_ORANGE;
...
static void cursor_switch_event_cb(lv_obj_t *obj, lv_event_t event)
{
    if (event == LV_EVENT_VALUE_CHANGED) {
        ...
        // 显示或隐藏光标相关控件
        if (cursorMode) {
            lv_obj_set_hidden(cursorSelect, false);
            lv_obj_set_hidden(cursorSlider, false);
        } else {
...
static void timebase_slider_event_cb(lv_obj_t *obj, lv_event_t event)
{
    if (event == LV_EVENT_VALUE_CHANGED) {
        timebaseIndex = lv_slider_get_value(obj);
...
static void amp_slider_event_cb(lv_obj_t *obj, lv_event_t event)
{
    if (event == LV_EVENT_VALUE_CHANGED) {
        scaleIndex = lv_slider_get_value(obj);
...
{
    if (event == LV_EVENT_CLICKED && isPaused) {
        if (obj == lv_obj_get_child(scrollBtns, 0)) {
            // 左滚动
            scrollOffset = min(scrollOffset + 10, MAX_SAMPLES - visibleSamples);
            Serial.println("向左滚动");
        } else {
            // 右滚动
            scrollOffset = max(scrollOffset - 10, 0);
            Serial.println("向右滚动");
        }
    }
}
...
void create_ui()
{
    ...
    timebaseSlider = lv_slider_create(timeTab, NULL);
    lv_obj_set_size(timebaseSlider, RIGHT_PANEL_WIDTH - 20, 20);
    lv_obj_align(timebaseSlider, timebaseLabel, LV_ALIGN_OUT_BOTTOM_MID, 0, 15);
    lv_obj_set_event_cb(timebaseSlider, timebase_slider_event_cb);
    lv_slider_set_range(timebaseSlider, 0, sizeof(timebases) / sizeof(timebases[0]) - 1);
    lv_slider_set_value(timebaseSlider, timebaseIndex, LV_ANIM_OFF);
    
    // 显示当前时基值的标签
    lv_obj_t *timeValueLabel = lv_label_create(timeTab, NULL);
    char time_txt[16];
    sprintf(time_txt, "%.1fms/div", currentTimebase);
    lv_label_set_text(timeValueLabel, time_txt);
    lv_obj_align(timeValueLabel, timebaseSlider, LV_ALIGN_OUT_BOTTOM_MID, 0, 15);
    ...
    ampSlider = lv_slider_create(ampTab, NULL);
    lv_obj_set_size(ampSlider, RIGHT_PANEL_WIDTH - 20, 20);
    lv_obj_align(ampSlider, ampLabel, LV_ALIGN_OUT_BOTTOM_MID, 0, 15);
    lv_obj_set_event_cb(ampSlider, amp_slider_event_cb);
    lv_slider_set_range(ampSlider, 0, sizeof(scaleSteps) / sizeof(scaleSteps[0]) - 1);
    lv_slider_set_value(ampSlider, scaleIndex, LV_ANIM_OFF);
    
    // 显示当前幅度值的标签
    lv_obj_t *ampValueLabel = lv_label_create(ampTab, NULL);
    char amp_txt[16];
    sprintf(amp_txt, "%.2fx", amplitudeScale);
    lv_label_set_text(ampValueLabel, amp_txt);
    lv_obj_align(ampValueLabel, ampSlider, LV_ALIGN_OUT_BOTTOM_MID, 0, 15);
    ...
    lv_obj_align(cursorSwitch, cursorModeLabel, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);
    
    // 创建光标选择下拉菜单
    #if LV_VERSION_CHECK(7, 0, 0)
        cursorSelect = lv_dropdown_create(cursorTab, NULL);
        lv_dropdown_set_options(cursorSelect, "光标1\n光标2");
    #else
    ...
    lv_obj_align(cursorSelect, cursorSwitch, LV_ALIGN_OUT_BOTTOM_MID, 0, 15);
    ...
    lv_obj_align(cursorSlider, cursorSelect, LV_ALIGN_OUT_BOTTOM_MID, 0, 15);
